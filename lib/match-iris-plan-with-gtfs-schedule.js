...
deepStrictEqual(
	_buildFindScheduleStopTimeQuery({
		...
		normalizedPrevStopName: 'tapfheim',
		normalizedNextStopName: 'genderkingen',
	}),
	{
		query: `\
SELECT
...
const createMatchIrisPlan = async (cfg, opt = {}) => {
	...
	const matchIrisPlanWithScheduleStopTimes = async (cfg) => {
		...
		// todo: use a log(n) data structure, e.g. a map
		const stopName = stations.find(s => s.evaNr === stopId)?.name ?? null
