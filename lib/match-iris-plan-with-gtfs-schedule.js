...
const createMatchIrisItems = async (cfg, opt = {}) => {
	...
	const matchIrisPlansAndChangesWithScheduleStopTimes = async (cfg) => {
		const {
			tripId,
			tripStart,
		} = cfg
		ok(tripId, 'cfg.tripId is missing/empty')
		ok(tripStart, 'cfg.rripStart is missing/empty')
		...
		const computeStopTimeMatchingParamsFromTimetable = (timetable) => {
			const {stopId, irisPlan} = timetable
		...
		// This means that `tripStart` & `arrPlannedPath[0]` *do not* always correspond with the 0th stop_time in GTFS schedule.
