...
`
	...
	const altServiceDate = (tripStart || serviceDate+'T').split('T')[0];
	if (altServiceDate != serviceDate) {
		query += `\
		OR date = $${paramsI++}
`
		params.push(altServiceDate);
	}
...
${_or ? 'OR ' : ''}(
	...
	AND stop_id LIKE $${paramsI++}
)
`
		params.push(
			stopSequenceInclSvcStops,
			escapeForLikeOp(stopId) + '%',
		)
		_or = true
	}
...
const createMatchIrisPlan = async (cfg, opt = {}) => {
	...
	const matchIrisPlanWithScheduleStopTimes = async (cfg) => {
		...
		const [stopName, ifopt] = stationNamesByEvaNr.get(stopId) ?? null
		...
		{
			{
				...
				const {
					...
					possibleRouteNames,
					tripStart,
					stopSequenceInclSvcStops: stopSequenceInclSvcStops-1,
					departure,
					stopId: ifopt,
					normalizedStopName,
					normalizedPrevStopName,
