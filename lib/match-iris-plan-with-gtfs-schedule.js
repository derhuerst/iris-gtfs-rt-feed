...
import {normalizeStopName} from './normalize-stop-name.js'
...
const _buildFindScheduleStopTimeQuery = (cfg) => {
	const {
		...
		stopId,
		normalizedStopNames,
		normalizedPrevStopNames,
		normalizedNextStopNames,
	} = cfg
	ok(Array.isArray(normalizedStopNames), 'cfg.normalizedStopNames must be an array')
	ok(Array.isArray(normalizedPrevStopNames), 'cfg.normalizedPrevStopNames must be an array')
	ok(Array.isArray(normalizedNextStopNames), 'cfg.normalizedNextStopNames must be an array')
...
`
	...
	if (stopSequenceInclSvcStops !== null && normalizedStopNames.length > 0) {
		query += `\
${_or ? 'OR ' : ''}(
	stop_sequence_consec = $${paramsI++}
	AND stop_name_normalized = ANY($${paramsI++})
)
`
		params.push(
			stopSequenceInclSvcStops,
	...
	if (normalizedPrevStopNames.length > 0 && normalizedStopNames.length > 0 && normalizedNextStopNames.length > 0) {
		query += `\
${_or ? 'OR ' : ''}(
	prev_stop_name_normalized = ANY($${paramsI++})
	AND stop_name_normalized = ANY($${paramsI++})
	AND next_stop_name_normalized = ANY($${paramsI++})
)
`
		params.push(
			normalizedPrevStopNames,
...
deepStrictEqual(
	_buildFindScheduleStopTimeQuery({
		...
		stopId: '8000078',
		normalizedStopNames: ['donauwörth'],
		normalizedPrevStopNames: ['tapfheim'],
		normalizedNextStopNames: ['genderkingen'],
	}),
	{
		query: `\
SELECT
...
OR (
	...
	AND stop_name_normalized = ANY($8)
)
OR (
	prev_stop_name_normalized = ANY($9)
	AND stop_name_normalized = ANY($10)
	AND next_stop_name_normalized = ANY($11)
)
OR (
	trip_start >= $12
	AND trip_start <= $13
...
const createMatchIrisPlan = async (cfg, opt = {}) => {
	...
	const matchIrisPlanWithScheduleStopTimes = async (cfg) => {
		...
		const normalizedStopNames = stop
			? [
				normalizeStopName(stop.name),
				normalizeStopName(stop.name, stop),
			]
			: []
		...
		const normalizedPrevStopNames = prevStopName
			? [
				normalizeStopName(prevStopName),
			]
			: []

		const depPlannedPath = irisPlan.departure?.planned_path ?? null
		const nextStopName = depPlannedPath ? depPlannedPath[0] : null
		const normalizedNextStopNames = nextStopName
			? [
				normalizeStopName(nextStopName),
			]
			: []
